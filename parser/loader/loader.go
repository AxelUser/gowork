...
func parseVacancyStats(page api.VacancySearchPage) []dataModels.VacancyStats {
	data := make([]dataModels.VacancyStats, len(page.Items))
	// Handle empty items and log!
	for i, v := range page.Items {
		data[i] = dataModels.NewVacancyStats(v.ID, v.URL, v.Salary.From, v.Salary.To, v.Salary.Currency)
	}
